...
void LlamaServer::load_model(const std::vector<std::string> &params) {
    ...
    const auto parsed = common_params_parse(
    ...
    try {
        ...
        auto example_format = common_chat_format_example(
            model_meta.chat_params.tmpls.get(),
...
int LlamaServer::request_completion(const std::string &json_params) {
    ...
    for (size_t i = 0; i < tokenized_prompts.size(); i++) {
        ...
        task.params = server_task::params_from_json_cmpl(
            ...
            ctx_server_->get_params_base(),
            ctx_server_->get_n_ctx_slot(),
            data);
...
std::vector<float> LlamaServer::embed(const std::string &prompt) {
    ...
    std::vector<std::vector<float>> embedding =
        out_res["embedding"].get<std::vector<std::vector<float>>>();
...
std::map<std::string, float> LlamaServer::rerank(
